...
RegisterNetEvent('qf_mdt:closeMdt', function ()
...
RegisterNetEvent('qf_mdt:openTablet', function ()
    if not PlayerData.job then
        return
    end
...
RegisterNUICallback('search', function(data, cb)
    if data.type == 'citizen' then
        if data.value then
            CALLBACK.TriggerServerCallback('qf_mdt:searchCitizen', function(info)
                Citizen.CreateThread(function()
                    SendNUIMessage({
            ...
            if player and player ~= -1 and distance < 5.0 then
                CALLBACK.TriggerServerCallback('qf_mdt:getCitizenIdentifier', function(info)
                    Citizen.CreateThread(function()
                        SendNUIMessage({
                            action = 'searchResults',
                            data = {
    ...
    elseif data.type == 'vehicle' then
        if data.value then
            CALLBACK.TriggerServerCallback('qf_mdt:searchVehicle', function(info)
                Citizen.CreateThread(function()
                    SendNUIMessage({
                        action = 'searchResults',
                        data = {
...
RegisterNUICallback('setData', function(data, cb)
    CALLBACK.TriggerServerCallback('qf_mdt:getData', function(callback)
        cb(callback)
    end, data)
end)
...
CLIENT.Load = function()
    CALLBACK.TriggerServerCallback('qf_mdt:getDispatchData', function(cb)
...
RegisterNetEvent('qf_mdt:updateAPWC', function(apwc)
...
RegisterNetEvent('qf_mdt:updateNapadowa', function(zt, data)
...
RegisterNetEvent('qf_mdt:updateZakladkaSV', function(zt, sv, ident)
    if zt == 'napadowa1' then
...
RegisterNetEvent('qf_mdt:updateZakladkaNEGO', function(zt, nego, ident)
    if zt == 'napadowa1' then
...
RegisterNetEvent('qf_mdt:updateZadania', function(zt, text)
    if zt == 'napadowa1' then
...
RegisterNetEvent('qf_mdt:updateNotatki', function(zt, text)
    if zt == 'napadowa1' then
...
RegisterNetEvent('qf_mdt:updatePlace', function(zt, text)
    if zt == 'napadowa1' then
...
RegisterNetEvent('qf_mdt:updateZatrzymani', function(text)
...
RegisterNetEvent('qf_mdt:updatePrzydzial', function(data)
...
RegisterNetEvent('qf_mdt:updateStatus', function(of, d)
...
RegisterNetEvent('qf-mdt:housesResults', function(data)
    SendNUIMessage({
...
RegisterNUICallback('setHouseData', function(data, cb)
    if data.houseName then
        CALLBACK.TriggerServerCallback('qf_mdt:getCurrentHouse', function(callback)
            cb(callback)
        end, data.houseName)
    end
end)
...
RegisterNUICallback('setWarrantData', function(data, cb)
    if data then
        CALLBACK.TriggerServerCallback('qf_mdt:currentWarrant', function(callback)
            cb(callback)
        end, data)
    end
end)

RegisterNetEvent('qf-mdt:updateWarrants', function(data)
    SendNUIMessage({
...
RegisterNetEvent('qf_mdt:searchEvidencesResult', function(data)
    SendNUIMessage({
...
RegisterNUICallback('setEvidenceData', function(data, cb)
    if data then
        CALLBACK.TriggerServerCallback('qf_mdt:currentEvidence', function(callback)
            cb(callback)
        end, data)
    end
end)
...
RegisterNetEvent('qf_mdt:updateSettings', function(data)
    if not data then
        SendNUIMessage({
...
RegisterNetEvent('qf_mdt:addDispatchAlert', function(gps, title, subtitle, code, color, response)
    if Config.Jobs.OnDuty[PlayerData.job.name] then
        CLIENT.lastDispatchNotif = CLIENT.lastDispatchNotif + 1
...
RegisterNetEvent('qf_mdt:DNaddreactionC', function(badge, notif, id)
    SendNUIMessage({
